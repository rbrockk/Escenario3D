...
class Light extends GLJPanel implements GLEventListener, KeyListener {
    ...
    private static String TITLE = "Aplicacion de iluminacion";  
    ...
    private final float[] radOrb = {1.5f, 2.0f, 2.5f, 3.0f};
    private float radioOrbita = 2.0f;  
    private int numOrbits = 4;      
    private final float[] inclinOrbita = {45.0f, 135.0f, 225.0f, 300.0f}; 
    ...
    public void setNeutron(GL2 gl, int face) {
        gl.glMaterialfv(face, GL2.GL_AMBIENT, colorBlue, 0);
        gl.glMaterialfv(face, GL2.GL_DIFFUSE, colorTurquoise, 0);
        gl.glMaterialfv(face, GL2.GL_SPECULAR, colorBlue, 0);
        gl.glMateriali(face, GL2.GL_SHININESS, 5);
        gl.glMaterialfv(face, GL2.GL_EMISSION, colorBlack, 0);
    }
    public void setProton(GL2 gl, int face) {
   ...
   public void drawFigure(GL2 gl, GLUT glut) {
    for (int i = 0; i < radOrb.length; i++) {
        float radioOrbita = radOrb[i];
        float inclinacion = inclinOrbita[i];
        
        gl.glPushMatrix();
        {
            this.setSomeGrayMaterial(gl, GL.GL_FRONT);
            gl.glRotatef(inclinacion, 1.0f, 0.0f, 0.0f); // Inclina la órbita en el eje X
            glut.glutWireTorus(0.05, radioOrbita, 30, 30);
        }
        gl.glPopMatrix();
    }

    // Dibuja esferas en cada órbita
    for (int i = 0; i < radOrb.length; i++) {
        float radioOrbita = radOrb[i];
        float inclinacion = inclinOrbita[i];
        float angle = 2 * (float) Math.PI / numOrbits; // Ajusta el ángulo según tus necesidades

        gl.glPushMatrix();
        {
            gl.glRotatef(inclinacion, 1.0f, 0.0f, 0.0f); // Inclina la órbita en el eje X
            gl.glTranslatef(radioOrbita * (float) Math.cos(angle), radioOrbita * (float) Math.sin(angle), 0.0f);
            this.setSomeYellowMaterial(gl, GL.GL_FRONT);
            gl.glTranslatef(0.0f, 0.0f, 0.0f);
            glut.glutSolidSphere(0.25, 30, 10);
        }
        gl.glPopMatrix();
    }   
    ...
    gl.glPushMatrix();
    {
        this.setProton(gl, GL.GL_FRONT);
        gl.glTranslatef(0.0f, 0.4f, 0.0f);
        glut.glutSolidSphere(0.4, 30, 10);
    }
    gl.glPopMatrix();
    ...
    gl.glPushMatrix();
    {
        this.setNeutron(gl, GL.GL_FRONT);
